...
* Added support for event batching via the event processor.
  ...
  * Added the `flush` method to allow flushing event_queue when required. When `flush` is called, any events waiting to be sent as part of a batched event request will be immediately batched and sent to the event dispatcher.
